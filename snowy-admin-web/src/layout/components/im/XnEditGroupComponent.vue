	...
	<div>
		<a-form ref="formRef" :model="formData" layout="vertical">
			...
			<a-form-item label="群聊人：" name="receiverIdList">
				<xn-im-user-selector :org-tree-api="selectorApiFunction.orgTreeApi"
					:user-page-api="selectorApiFunction.userPageApi"
					:user-list-by-id-list-api="selectorApiFunction.checkedUserListApi" data-type="object"
					v-model:value="formData.receiverIdList" :userShow="props.createGroupType == 'add'"
				...
				<a-list :data-source="userList">
					<template #renderItem="{ item }">
						<a-list-item v-for="key in [translateUser(item)]" :key="key">
							<template #actions>
								<a key="list-loadmore-edit" v-if="key.role == '3' && userId != key.userId && currentUser.role == '1'"
									@click="updateGroupMember(1, key.userId)">设为管理员</a>
								<a key="list-loadmore-more" v-if="key.role == '2' && userId != key.userId && currentUser.role == '1'"
									@click="updateGroupMember(2, key.userId)">取消管理员</a>
								<a key="list-loadmore-more" v-if="key.role != '1' && userId != key.userId && currentUser.role == '1'"
									@click="updateGroupMember(3, key.userId)">转让群主</a>
								<a key="list-loadmore-more" v-if="key.role != '1' && userId != key.userId && isShow(key.role)"
									@click="updateGroupMember(4, key.userId)">移除</a>
								<a key="list-loadmore-more"
									v-if="key.role != '1' && userId != key.userId && isShow(key.role) && !cancelSilenceTime(key.silenceTime)"
									@click="mutedShowOpen(key.userId)">禁言</a>
								<a key="list-loadmore-more"
									v-if="key.role != '1' && userId != key.userId && isShow(key.role) && cancelSilenceTime(key.silenceTime)"
									@click="cancelSilence(key.userId)">取消禁言</a>
							</template>
		...
		<div class="footer">
			...
			<a-button class="ml" type="primary" @click="add"
				v-if="currentUser.role == '1' || props.createGroupType == 'add'">保存</a-button>
			<a-button class="ml" type="primary" @click="add" v-if="userList.length != imGroupMembers.length">确认邀请</a-button>
...
const uploadLogo = () => {
	if (props.createGroupType == 'details' || (currentUser.role && currentUser.role != '1')) {
		notification.error({
			message: '无权限修改用户头像'
		})
		return
...
const cancelSilence = (userId) => {
	Modal.confirm({
		...
		onOk() {
			...
			imGroupMemberApi.imGroupMemberUnMute({
				id,
				userId
			}).then(() => {
...
const mutedShowOpen = (userId) => {
	if (typeof userId == 'string') {
		...
		imGroupMemberApi.imGroupMemberMute({
			id,
			silenceTime: time,
...
const isShow = (role) => {
	...
	role = role.toString();
	return (currentUser.value.role == '2' && role == '3') || (currentUser.value.role == '1' && ['2', '3'].indexOf(role) > -1);
...
const deleteGroup = () => {
	if (props.createGroupType == 'update') {
		Modal.confirm({
			...
			onOk() {
				imGroupApi.imGroupDelete([{ id: props.id }]).then(() => {
					notification.success({
						message: '解散成功'
					})
					emit('restChatUser')
...
const updateGroupMember = (type, toUserId) => {
	...
	//设为管理员
	if (type == 1) {
		imGroupMemberApi.imGroupMemberSubmitForm({
			id,
			userId: toUserId,
			role: 2
		}, true).then(() => {
		...
		Modal.confirm({
			...
			onOk() {
				imGroupMemberApi.imGroupMemberDelete([{ id }]).then(() => {
					notification.success({
						message: '移除成功'
					})
...
const add = () => {
	...
	// 创建群聊
	imGroupApi.imGroupSubmitForm(params, props.createGroupType == 'update' || props.createGroupType == 'details').then(() => {
		notification.success({
