  ...
  try {
    ...
    const message = {
      ...
      toUserId: props.targetUser.id,
			...
			toUserType: '1', //暂定写死，后期需要使用client端处理。
			fromUserType: '1',
      content: {
        status,
        duration,
		...
		if(props.targetUser.id&&props.currentUser.id){
...
// 监听远程视频流变化
watch(() => props.callState.remoteStream, (newStream) => {
  if (newStream) {
    // 确保两个视频元素都能播放远程流
    const mainVideo = document.getElementById('remoteVideo') as HTMLVideoElement;
    const smallVideo = document.getElementById('remoteVideoSmall') as HTMLVideoElement;
    
    if (mainVideo) {
      mainVideo.srcObject = newStream;
      mainVideo.play().catch(e => console.error('主窗口视频播放失败:', e));
    }
    if (smallVideo) {
      smallVideo.srcObject = newStream;
      smallVideo.play().catch(e => console.error('小窗口视频播放失败:', e));
    }
  }
}, { immediate: true });
...
const switchVideo = (type: 'local' | 'remote') => {
  ...
  nextTick(() => {
    ...
    if (mainVideo && props.callState.remoteStream) {
      mainVideo.srcObject = props.callState.remoteStream;
      mainVideo.play().catch(e => console.error('主窗口视频播放失败:', e));
    }
    if (smallVideo && props.callState.remoteStream) {
      smallVideo.srcObject = props.callState.remoteStream;
      smallVideo.play().catch(e => console.error('小窗口视频播放失败:', e));
    }
  });
};
...
</style> c o n s o l e . l o g ( ' �݋!jWW  i n c o m i n g C a l l : ' ,   c a l l S e r v i c e . s t a t e . i n c o m i n g C a l l ) 
 
 