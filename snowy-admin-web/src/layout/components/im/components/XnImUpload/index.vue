...
<script setup name="uploadIndex">
	...
	const action =
		props.uploadResultType === 'id'
			? props.uri + convertUrl(props.uploadReturnIdApi)
			: props.uri + convertUrl(props.uploadDynamicReturnUrlApi)

	// 构造文件对象
	const buildFileObject = (url, id) => {
		return {
			data: url ? url : props.uri + convertUrl(props.uploadIdDownloadUrl) + id,
			name: url ? url : id,
			url: url ? url : props.uri + convertUrl(props.uploadIdDownloadUrl) + id,
			status: 'done',
			response: {
				data: url ? url : id,
				code: 200
		...
		if (props.uploadResultCategory === 'array') {
			if (props.completeResult) {
				...
				newResult.map((e) => {
					...
					if (props.uploadResultType === 'id') {
						e.url = props.uri + convertUrl(props.uploadIdDownloadUrl) + e.response.data
					}
					if (props.uploadResultType === 'url') {
	...
	const handlePreview = async (file) => {
		...
		// 如果返回的是id
		if (props.uploadResultType === 'id') {
			previewImage.value = props.uri + convertUrl(props.uploadIdDownloadUrl) + file.response.data
		} else {
