...
// HTTP request 拦截器 - 从 CLIENT_TOKEN 读取token
clientService.interceptors.request.use(
	(config) => {
		const token = tool.data.get('CLIENT_TOKEN')
		if (token) {
...
// 适配器, 用于适配不同的请求方式
export const clientBaseRequest = (url, value = {}, method = 'post', options = {}) => {
	if (method === 'post') {
		return clientService.post(url, value, options)
	} else if (method === 'get') {
		return clientService.get(url, { params: value, ...options })
	} else if (method === 'formdata') {
		// form-data表单提交的方式
		return clientService.post(url, qs.stringify(value), {
			headers: {
