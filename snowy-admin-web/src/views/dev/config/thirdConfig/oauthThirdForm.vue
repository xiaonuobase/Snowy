		...
		>
			<a-row :gutter="8">
				<a-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
					<a-form-item label="是否允许OAUTH登录：" name="SNOWY_THIRD_OAUTH_ALLOW_LOGIN_FLAG">
						<a-switch
							v-model:checked="formData.SNOWY_THIRD_OAUTH_ALLOW_LOGIN_FLAG"
							checked-children="是"
							un-checked-children="否"
							placeholder="请选择是否允许OAUTH登录"
						/>
					</a-form-item>
				</a-col>
				<a-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
				...
				<a-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
					<a-form-item name="SNOWY_THIRD_OAUTH_REDIRECT_URL" v-if="formData.SNOWY_THIRD_OAUTH_ALLOW_LOGIN_FLAG">
						<template #label>
							<a-tooltip>
								<template #title> 请填写{前端地址}/callback/OAUTH，如http://localhost:81/callback/OAUTH </template>
								<question-circle-outlined />
							</a-tooltip>
							重定向地址：
						</template>
						<a-input v-model:value="formData.SNOWY_THIRD_OAUTH_REDIRECT_URL" placeholder="请输入重定向URL" />
					</a-form-item>
				</a-col>
				<a-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
					<a-form-item name="SNOWY_THIRD_OAUTH_SOURCE_PROPERTY" v-if="formData.SNOWY_THIRD_OAUTH_ALLOW_LOGIN_FLAG">
						<template #label>
							<a-tooltip>
						...
						<a-input v-model:value="formData.SNOWY_THIRD_OAUTH_SOURCE_PROPERTY" placeholder="请输入关联源属性" />
					</a-form-item>
				</a-col>
				<a-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
					<a-form-item name="SNOWY_THIRD_OAUTH_TARGET_PROPERTY" v-if="formData.SNOWY_THIRD_OAUTH_ALLOW_LOGIN_FLAG">
						<template #label>
							<a-tooltip>
						...
						<a-input v-model:value="formData.SNOWY_THIRD_OAUTH_TARGET_PROPERTY" placeholder="请输入关联目标属性" />
					</a-form-item>
				</a-col>
				<a-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
					<a-form-item>
...
<script setup name="oauthThirdForm">
	...
	// 默认要校验的
	const formRules = {
		SNOWY_THIRD_OAUTH_ALLOW_LOGIN_FLAG: [required('请选择是否允许OAUTH登录')],
		SNOWY_THIRD_OAUTH_AUTHORIZE_URL: [required('请输入OAUTH认证地址')],
		SNOWY_THIRD_OAUTH_ACCESS_TOKEN_URL: [required('请输入OAUTH获取token地址')],
		SNOWY_THIRD_OAUTH_USER_INFO_URL: [required('请输入OAUTH获取用户信息地址')],
		SNOWY_THIRD_OAUTH_SCOPE: [required('请输入OAUTH权限范围')],
		SNOWY_THIRD_OAUTH_CLIENT_ID: [required('请输入OAUTH客户端ID')],
		SNOWY_THIRD_OAUTH_CLIENT_SECRET: [required('请输入OAUTH客户端SECRET')],
		SNOWY_THIRD_OAUTH_REDIRECT_URL: [required('请输入重定向URL')],
		SNOWY_THIRD_OAUTH_SOURCE_PROPERTY: [required('请输入OAUTH关联源属性')],
		SNOWY_THIRD_OAUTH_TARGET_PROPERTY: [required('请输入OAUTH关联目标属性')]
	}
	// 验证并提交数据
	const onSubmit = () => {
		formRef.value
