		...
		<a-form ref="formRef" :model="formData" :rules="formRules" layout="vertical">
			<a-form-item label="上级机构：" name="parentId">
				<xn-tree-select
					...
					:ancestor-api="bizOrgApi.orgGetAncestorNodes"
			...
			<a-form-item label="机构编码：" name="code">
				<a-input v-model:value="formData.code" placeholder="请输入机构编码" allow-clear />
			</a-form-item>
...
<script setup name="bizOrgForm">
	...
	const onOpen = (record, parentId) => {
		...
		nextTick(() => {
			if (record) {
				bizOrgApi.orgDetail({ id: record.id }).then((data) => {
	...
	const selectorApiFunction = {
		orgTreeApi: (param) => bizOrgApi.orgTreeSelector(param),
		userPageApi: (param) => bizOrgApi.orgUserSelector(param)
	}
